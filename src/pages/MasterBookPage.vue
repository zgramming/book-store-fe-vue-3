...
const {
  data: masterBooks,
  isValidating,
  mutate: reloadMasterBooks,
} = useSWRV(`/master-books`, async () =>
  masterBookService.getAllBooks({
    limit: limit.value,
    page: page.value,
    title: qTitle.value,
  }),
...
const handleNew = () => {
  router.push('/master-book/new');
};

const handleEdit = (id: number) => {
  router.push(`/master-book/${id}`);
};

const handleDelete = async (id: number) => {
  try {
    await masterBookService.deleteBook(`${id}`);
  ...
  <base-layout>
    ...
    <div class="tw-flex tw-flex-col tw-gap-5">
      ...
      <v-table fixed-header>
        ...
        <tbody>
          <tr v-for="(item, index) in masterBooks?.data" :key="item.id">
            <td>{{ index + 1 }}</td>
            <td>{{ item.title }}</td>
            <td>{{ item.author }}</td>
